...
public class SampleRepositoryMockImpl implements SampleRepository {
    ...
    private static final Supplier<Stream<Sample>> SAMPLE_STREAM_SUPPLIER = () ->
            Stream.of(new Sample(1, "outro", 1),
    ...
    public Optional<Sample> findById(Integer integer) {
        return SAMPLE_STREAM_SUPPLIER.get().filter(sample -> sample.getId().equals(integer)).findAny();
    }
    ...
    public Iterable<Sample> findAll() {
        return SAMPLE_STREAM_SUPPLIER.get().collect(Collectors.toList());
    }
    ...
    public Iterable<Sample> findAllById(Iterable<Integer> iterable) {
        ...
        return SAMPLE_STREAM_SUPPLIER.get().filter(sample -> ids.contains(sample.getId())).collect(Collectors.toList());
    }
